...
function opcionToString(opt) {
  ...
  if (opt == null) return '';
  return (
    opt.valor ?? opt.texto ?? opt.text ?? String(opt.id ?? JSON.stringify(opt))
  );
}
...
function normalizeOption(o) {
  ...
  if (typeof o === 'object')
    return {
      ...o,
      valor: String(o.valor ?? o.texto ?? o.text ?? (o.id != null ? o.id : '')),
    };
  return { valor: String(o) };
}

export default function CuestionarioConfigPage({
  title = 'Configuración de cuestionario',
}) {
    ...
    setPreguntas(
      (all || []).map((p) => ({
        ...
        opciones: Array.isArray(p.opciones)
          ? p.opciones.map(normalizeOption)
          : [],
        tipo: p.tipo ?? 'TEXTO',
      }))
    );
  ...
  const handleCreate = async () => {
    ...
    console.log(
      'Creating pregunta with JSON payload:',
    ...
    setPreguntas((prev) => [
      {
        ...saved,
        opciones: Array.isArray(saved.opciones)
          ? saved.opciones.map(normalizeOption)
          : [],
      },
      ...prev,
    ]);
  ...
  const handleUpdate = async (p) => {
    ...
    const opcionesRaw = (Array.isArray(p.opciones) ? p.opciones : []).map(
      normalizeOption
    );
    ...
    const opcionesClean = opcionesStrings.filter(
      (v, i, a) => a.indexOf(v) === i
    );
    if (opcionesStrings.length !== opcionesClean.length) {
      console.warn(
        'Se eliminaron opciones duplicadas o vacías antes de enviar:',
        { before: opcionesStrings, after: opcionesClean }
      );
    }
    ...
    try {
      ...
      const updatedOpciones = Array.isArray(updated.opciones)
        ? updated.opciones
        : opcionesRaw;
      const mergedOpciones = updatedOpciones.map((uo, idx) => {
        ...
        const found = opcionesRaw.find((or) => {
          ...
          if (typeof or === 'object' && or.id != null)
            return String(or.id) === String(uo);
          return String(or.valor ?? or) === String(uo);
        });
        if (found) return found;
        if (
          !isNaN(Number(uo)) &&
          opcionesRaw.length === updatedOpciones.length
        ) {
      ...
      setPreguntas((prev) =>
        prev.map((x) =>
          x.id === p.id ? { ...updated, opciones: mergedOpciones } : x
        )
      );
      return updated;
    } catch (err) {
      ...
      if (payload.opciones && payload.opciones.length) {
        try {
          const altOpciones = opcionesRaw
            .map((o) => (o?.id ? String(o.id) : String(o.valor ?? o)))
            .filter((s) => s.trim() !== '');
          const altPayload = {
            valor: p.valor,
            tipo: p.tipo,
            obligatoria: p.obligatoria,
            opciones: altOpciones,
          };
          console.log('Reintentando update con payload alternativo:', {
            id: p.id,
            altPayload,
          });
          const updatedAlt = await preguntasService.update(p.id, altPayload);

          const updatedOpciones = Array.isArray(updatedAlt.opciones)
            ? updatedAlt.opciones
            : opcionesRaw;
          const mergedOpciones = updatedOpciones.map((uo, idx) => {
            if (uo == null) return uo;
          ...
          setPreguntas((prev) =>
            prev.map((x) =>
              x.id === p.id ? { ...updatedAlt, opciones: mergedOpciones } : x
            )
          );
          return updatedAlt;
        } catch (err2) {
  ...
  const callDeleteService = async (id) => {
    ...
    for (const m of methods) {
      if (typeof preguntasService[m] === 'function') {
        ...
        throw new Error(
          `No se pudo eliminar usando preguntasService.${m} (probadas firmas comunes).`
        );
      }
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (err?.response?.status === 404) {
        console.warn(
          `Pregunta ${id} no encontrada en servidor (404). Se elimina localmente.`
        );
        setPreguntas((prev) => prev.filter((x) => x.id !== id));
        return;
      }
      if (err?.response?.status === 400) {
        console.warn(
          `Bad Request (400) al eliminar pregunta ${id}. Revisa la firma del servicio o el payload en el backend.`
        );
        return;
      }
                  ...
                  <tr key={p.id ?? `row-${idx}`} className="abmc-row">
                    ...
                    <td>
                      <div className="abmc-actions">
                        ...
                        <button
                          type="button"
                          className="abmc-btn abmc-btn-danger"
                          onClick={() => handleDelete(p.id)}
                        >
                          {' '}
                          <TrashIcon fill="var(--text-light)" />
                        </button>
                        {isAsignada ? (
                          <button
                            type="button"
                            className="abmc-btn abmc-btn-danger"
                            title="Quitar del cuestionario"
                            onClick={() => handleQuitar(p.id)}
                          >
                            <RemoveIcon fill="var(--text-light)" />
                          </button>
                        ) : (
                          <button
                            type="button"
        ...
        {isEditing && editing ? (
          ...
          >
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              {(editing.tipo === 'OPCION' ||
                editing.tipo === 'MULTIOPCION') && (
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  <input
                    className="abmc-input"
                    placeholder="Nueva opción"
                    value={editOpcionInput}
                    onChange={(e) => setEditOpcionInput(e.target.value)}
                  />
                  <button
                    className="abmc-btn abmc-btn-primary"
                    type="button"
