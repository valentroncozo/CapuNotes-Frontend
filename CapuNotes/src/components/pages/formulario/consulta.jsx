...
function formatFecha(fechaIso) {
  ...
  if (isNaN(date)) return fechaIso; // por si viene algo raro
...
export default function FormularioConsulta({
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (inscripcionId) {
          const perfil = await AudicionService.getPerfilInscripcion(
            inscripcionId
          );
          ...
          const encuesta = await preguntasService.getFormulario(
            perfil.audicionId
          );
          ...
          const unmatched = raw.filter((r) => {
            const idMatch = normalized.some(
              (n) =>
                String(n.preguntaId) === String(r.preguntaId) ||
                String(n.preguntaId) === String(r.id)
            );
            const textMatch = normalized.some((n) => {
              const rawText = String(r.pregunta ?? '')
                .trim()
                .toLowerCase();
              const p = encuesta?.find(
                (q) => String(q.id) === String(n.preguntaId)
              );
              return (
                p &&
                String(p.valor ?? '')
          ...
          if (unmatched.length)
            console.warn('Respuestas sin match en encuesta:', unmatched);

          setRespuestas(normalized);

          // normalizar turno si viene en perfil
          if (perfil?.turnoId) {
            const t = (encuesta.turnos || []).find(
              (x) => String(x.id) === String(perfil.turnoId)
            );
            setSeleccionTurno(
              t ? `${t.diaString || t.fecha || ''} ${t.horaInicio || ''}` : null
            );
          } else if (perfil?.turnoDia || perfil?.turnoHora) {
            setSeleccionTurno(
              `${perfil.turnoDia ?? ''} ${perfil.turnoHora ?? ''}`.trim()
            );
          }
        }
      } catch (err) {
  ...
  const lookupRespuesta = (preguntaId) =>
    respuestas.find((r) => String(r.preguntaId) === String(preguntaId)) ?? null;
    ...
    <main className="encuesta-container">
      <div className="abmc-card">
        ...
        {loading ? (
          ...
          <form
            className="cuestionario-formulario"
          ...
          >
            <section className="container-candidato">
              <h2>
                Datos Personales{' '}
                <span>
                  Inscripciones anteriores: {Number(cantidadAudiciones) - 1}{' '}
                </span>
              </h2>
              ...
              <div className="mitad">
                <div className="form-group-miembro">
                  ...
                  <input
                    type="text"
                    className="abmc-input"
                    value={formatFecha(candidato?.fechaNacimiento)}
                    disabled
                  />
                </div>
                <div className="form-group-miembro">
