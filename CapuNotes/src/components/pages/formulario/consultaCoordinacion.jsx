...
export default function FormularioConsultaCoordinacion({ title = 'Consulta de Inscripción' }) {
  ...
  const [selectedCuerda, setSelectedCuerda] = useState('');
  const [updatingCuerda, setUpdatingCuerda] = useState(false);
  ...
  useEffect(() => {
    // inicializar select cuando llegue el candidato
    setSelectedCuerda(candidato?.cuerda ?? '');
  }, [candidato]);

  const lookupRespuesta = (preguntaId) => respuestas.find(r => String(r.preguntaId) === String(preguntaId)) ?? null;

  const handlerAsignarCuerda = (cuerdaSeleccionada) => {
    // Aquí puedes manejar la lógica para asignar la cuerda al candidato
    console.log(`Cuerda asignada: ${cuerdaSeleccionada}`);
  };
        ...
        {loading ? (
          ...
          <form className='cuestionario-formulario' onSubmit={(e) => e.preventDefault()}>
            <section className='container-candidato'>
              <h2>Datos Personales   <span>Inscripciones anteriores: {Math.max(0, Number(cantidadAudiciones ?? 0) - 1)} </span></h2>
            ...
            <div className='form-group-miembro'>
              <label>Asignar Cuerda al candidato</label>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <select
                  className='abmc-input'
                  value={selectedCuerda ?? ''}
                  onChange={(e) => setSelectedCuerda(e.target.value)}
                >
                  <option value="">-</option>
                  {cuerdas.map((c) => (
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      setUpdatingCuerda(true);
                      await incripcionService.actualizarCuerda(inscripcionId, selectedCuerda);
                      setCandidato(prev => ({ ...(prev ?? {}), cuerda: selectedCuerda }));
                      Swal.fire({ icon: 'success', title: 'Cuerda actualizada', text: `Cuerda: ${selectedCuerda}` });
                    } catch (err) {
                      console.error('Error actualizando cuerda:', err);
                      Swal.fire({ icon: 'error', title: 'Error', text: err?.response?.data?.message ?? 'No se pudo actualizar la cuerda.' });
                    } finally {
                      setUpdatingCuerda(false);
                    }
                  }}
                >
                  {updatingCuerda ? 'Guardando...' : 'Asignar'}
                </button>
              </div>
            </div>
