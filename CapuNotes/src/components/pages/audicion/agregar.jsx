...
const AudicionAgregar = ({title="Agregar Audición"}) => {
  ...
  // Estado 'data' con estructura: { nombre, ubicacion, fechaDesde, fechaHasta, dias: { "DD/MM/YYYY": [ { horaDesde, horaHasta, duracion } ] } }
  ...
  const handlerCancelar = () => {
    ...
    setUbicacion('');
    setDias([]);
    setData({ nombre, ubicacion: '', fechaDesde: '', fechaHasta: '', dias: {} });
  };
  ...
  const handleGuardarBorrador = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: error?.message || "No se pudo registrar la audición",
        background: "#11103a",
        color: "#E8EAED",
      });
    } finally {
      ...
      <div className="abmc-card">
        ...
        <section className="content-form-audicion">
          <form className="form-audicion" onSubmit={handlerObtenerDias}>
            ...
            <div className="content-inputs-date-audicion">
              ...
              <div className="button-date-audicion">
                <button type="submit" className="abmc-btn btn-primary">
                  Agregar Fechas
                </button>
              </div>
            </div>
          ...
          <section className="content-turno-input">
            {dias.length > 0 ? (
              dias.map((dia, index) => {
                const dayKey = formatDDMMYYYY(dia); // clave estable por día
                return (
                  <TurnoSection
                    key={dayKey}
                    index={index}
