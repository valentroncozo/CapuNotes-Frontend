...
const parseEventDate = (evento) => {
  ...
  // base sin desfase
  const [y, m, d] = evento.fechaInicio.split("-").map(Number);
  const dt = new Date(y, m - 1, d);

  // si tiene hora, la seteamos local
  if (evento.hora) {
    const [h, min, s] = evento.hora.split(":").map(Number);
...
export default function Principal({ username }) {
  ...
  const handlePdfClick = () => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        const url = `${lecturaUrl}${lecturaUrl.includes("?") ? "&" : "?"
          }print=1`;
          ...
          <div className="eventos-header">
            ...
            <div className="eventos-actions">
              <button
                ...
                title="Ver QR del repertorio evento próximo"
              >
              ...
              <button
                ...
                title="Descargar PDF del repertorio evento próximo"
              >
            ...
            {loadingEventos ? (
              <div
                style={{
                  ...
                  alignItems: "center",
                  padding: "0.5rem 0",
                  height: "120px", // mismo alto que una card
                }}
              >
              ...
              upcomingEventos.slice(0, 5).map((evento) => (
                <div className="evento-card" key={evento.id}>
                  ...
                  <div className="evento-info">
                    ...
                    <p>{formatFechaDDMMYYYY(evento.fechaInicio)}</p>
                  </div>
                  ...
                  <div className="evento-info">
                    ...
                    <p>{formatHoraHHMM(evento.hora)}</p>
                  </div>
